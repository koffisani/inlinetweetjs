...
if ( inlineTweets ) {
	...
	// Class that'll fetch Bitly shorten Url
	...
	function buildInlineTweet(inlineTweet, accessToken) {
		...
		var bitlyRequest = "https://api-ssl.bitly.com/v3/shorten?access_token=" + accessToken + "&longUrl=" + currentUrl;
		var client = new HttpClient(),
		response = 'depart';
		console.log(bitlyRequest);
		client.get(bitlyRequest, function(reponse) {
			response = reponse;
		});
		response = JSON.parse(response);
		var pageURL = '';
		console.log(response);
		if(inlineTweet.dataset.inlineTweetUrl){
			pageURL = inlineTweet.dataset.inlineTweetUrl;
		}
	...
	for ( var i = 0; i < inlineTweets.length; i++ ) {
		buildInlineTweet(inlineTweets[i], ACCESS_TOKEN);
	}
